...
export default function getCSSVariables<T extends BaseConfig>(widgetSettings: T): string {
  const cssVariables: { [key: string]: string } = {
    ...
    "--max-tile-width": `${widgetSettings.max_tile_width ?? 0}px`,
    "--margin": `${widgetSettings.margin ?? 0}px`,
    ...
    "--text-tile-user-handle-font-size": `${widgetSettings.text_tile_user_handle_font_size ?? 12}px`,
    ...
    "--tile-tag-inline-background": `#${widgetSettings.tile_tag_inline_background ?? "#00000066"}`,
