...
export function getProductHeaderDiv(product: TagExtended): string {
  ...
  const priceContent = price ? `<div class="stacklapopup-products-item-price">${price}</div>` : ""
...
export function getProductDetailsDiv(sdk: Sdk): string {
  ...
  return products.map((product: TagExtended) => renderProductDetailsDiv(sdk, product)).join("")
}

export function renderProductDetailsDiv(sdk: Sdk, product: TagExtended): string {
  ...
  let callToAction = ""
  const { custom_url, target, description, availability, cta_text = "Buy Now", currency, id } = product
...
export function renderCrossSellersDiv(products: TagExtended[], selectedProductId: string) {
...
export default function productsTemplate(sdk: Sdk): string {
  ...
  const products: TagExtended[] = (tile.tags_extended || []).filter((tag: TagExtended) => tag.type === "product")

  if (!products) {
    return ""
  }

  const selectedProductById = products.find(({ id }) => id == productId.toString())
